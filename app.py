...
# ══════════════════════════════════════════════════════════════════════════════
ROLES = {
"Academic Advisor — FESAC" : {"faculty":"FESAC","icon":" ","pwd_key":"ADVISOR_FESAC_PASSWORD",""Academic Advisor — FBA" : {"faculty":"FBA", "icon":" ","pwd_key":"ADVISOR_FBA_PASSWORD", "Academic Advisor — FEHAS" : {"faculty":"FEHAS","icon":" ","pwd_key":"ADVISOR_FEHAS_PASSWORD",""Academic Advisor — PSTM" : {"faculty":"PSTM", "icon":" ","pwd_key":"ADVISOR_PSTM_PASSWORD", "Head of Department — FESAC":{"faculty":"FESAC","icon":" ", "pwd_key":"HOD_FESAC_PASSWORD", "Head of Department — FBA" :{"faculty":"FBA", "icon":" ", "pwd_key":"HOD_FBA_PASSWORD", "Head of Department — FEHAS":{"faculty":"FEHAS","icon":" ", "pwd_key":"HOD_FEHAS_PASSWORD", "Head of Department — PSTM" :{"faculty":"PSTM", "icon":" ", "pwd_key":"HOD_PSTM_PASSWORD", "Dean of Students" :{"faculty":None, "icon":" ", "pwd_key":"DEAN_PASSWORD", }
# ══════════════════════════════════════════════════════════════════════════════
# CSS — clean, simple, friendly
# ══════════════════════════════════════════════════════════════════════════════
st.markdown("""
<style>
@import url('https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700;800&display=:root{
...
@keyframes fadeUp{from{opacity:0;transform:translateY(6px)}to{opacity:1;transform:translateY(.block-container > div { animation: fadeUp .35s ease-out; }
/* ── Sidebar ── */
...
return (" ", f"{name} is projected to remain in the "
...
"Confidence", f"{max(row.get('prob_low',0),row.get('prob_med',0),row.get('prob_high',["Low Risk", f"{row.get('prob_low',0):.0%}",
...
drawn = 0
for fac in FACULTIES:
sub = df[(df["faculty"]==fac)&(df["risk_class"]==2)] if "faculty" in df.if len(sub) == 0:
continue
...
ok = " " if pct >= 50 else " " if pct >= 40 else " "
st.markdown(f"""
...
(" ", "Sems Below 1.5 GPA", f"{int(row.get('consec_fails',0))}", "#f1f5f9"),
...
'<div style="font-size:.82rem;color:#94a3b8">High Risk: <b style="color:' orig_rc + '">' + f"{orig_probs[2]:.0%}" + '</b></div></div>',
unsafe_allow_html=True)
with comp_sep:
st.markdown(
...
'<div style="font-size:.82rem;color:#94a3b8">High Risk: <b style="color:' sim_rc + '">' + f"{sim_probs[2]:.0%}" + '</b></div></div>',
unsafe_allow_html=True)
if sim_pred < orig_pred:
icon_c, clr = "\u2705", "#22c55e"
msg = (f"These changes improve the prediction from <b>{RISK_LABEL[orig_pred]}</f" to <b style='color:{sim_rc}'>{RISK_LABEL[sim_pred]}</b>. "
f"High Risk probability drops by <b style='color:#22c55e'>{abs(delta):.elif sim_pred > orig_pred:
icon_c, clr = "\u26a0\ufe0f", "#f59e0b"
msg = (f"These values would worsen risk to <b style='color:{sim_rc}'>"
f"{RISK_LABEL[sim_pred]}</b>. "
f"High Risk probability rises by <b style='color:#ef4444'>{abs(delta):.else:
...
<span class="pill"> {fac_display}</span>
...
st.markdown('<p style="color:#f1f5f9;font-weight:700;'
'font-size:.92rem;margin-bottom:.3rem">'
' Is the AI Fair to All Students?</p>',
unsafe_allow_html=True)
st.markdown('<p style="color:#94a3b8;font-size:.8rem;'
'margin-bottom:.5rem">'
...
line-height:1.65">
<b style="color:#f1f5f9">Important:</b>
...
<b style="color:{fac_color}"> Faculty Filter Active</b><br>
...
(k1, " ", n, "Students Analysed", "Full cohort", "#60a5fa", "rgba(96,165,250,.(k2, " ", n_hr, "High Risk", "Need immediate attention", "#ef4444", "rgba(239,68,68,.(k3, " ", n_mr, "Medium Risk", "Need monitoring", "#f59e0b", "rgba(245,158,11,.(k4, " ", n_lr, "Low Risk", "On track", "#22c55e", "rgba(34,197,94,.]:
with col:
st.markdown(f"""
...
st.markdown('<p style="color:#f1f5f9;font-size:.95rem;font-weight:600;'
'margin:.5rem 0"> Faculty Performance Comparison</p>',
unsafe_allow_html=True)
