...
# INSIGHTS
# ══════════════════════════════════════════════════════════════════════════════
def generate_insights(df: pd.DataFrame, faculty_filter) -> list:
    """
    Compute plain-English statistical insights from batch prediction results.
    ...
    if "gender" in df.columns and "semester_gpa" in df.columns:
        ...
        if "Female" in genders and "Male" in genders:
            ...
            if f_gpa >= m_gpa:
                higher_gender, higher_gpa = "Female", f_gpa
                lower_gender, lower_gpa = "Male", m_gpa
            else:
                higher_gender, higher_gpa = "Male", m_gpa
                lower_gender, lower_gpa = "Female", f_gpa
            gap = abs(f_gpa - m_gpa)
            ...
            insights.append((
                ...
                f"({gap:.2f} GPA points)",
                f"{higher_gender} students average GPA {higher_gpa:.2f} vs "
                f"{lower_gender} students at {lower_gpa:.2f}. "
...
def generate_insights(df: pd.DataFrame, faculty_filter) -> list:
    ...
    if "gender" in df.columns and "semester_gpa" in df.columns:
        ...
        if "Female" in genders and "Male" in genders:
            f_gpa = df[df["gender"].str.strip().str.title()=="Female"]["semester_gpa"].mean()
            m_gpa = df[df["gender"].str.strip().str.title()=="Male"]["semester_gpa"].mean()
            higher, lower = ("Female", f_gpa, "Male", m_gpa) if f_gpa >= m_gpa                              else ("Male", m_gpa, "Female", f_gpa)
            gap = abs(f_gpa - m_gpa)
            color = "#22c55e" if gap < 0.15 else "#f59e0b"
            insights.append((
                "👥", color,
                f"{'Minimal' if gap < 0.15 else 'Moderate'} gender performance gap "
                f"({gap:.2f} GPA points)",
                f"{higher[0]} students average GPA {higher[1]:.2f} vs "
                f"{lower[2]} students at {lower[3]:.2f}. "
                f"{'The gap is small — the model treats both groups equitably.' if gap < 0.15 else 'This gap warrants investigation to ensure equitable academic support.'}"
            ))
...
def generate_cohort_pdf(df: pd.DataFrame, role: str, faculty_filter) -> bytes:
    """
    Generate a formatted cohort summary PDF for HODs/Dean to take to meetings.
    Includes: summary stats, risk by faculty table, top at-risk students,
    gender breakdown, and graduation projections if available.
    """
    try:
        from reportlab.lib.pagesizes import A4
        from reportlab.lib.styles    import getSampleStyleSheet, ParagraphStyle
        ...
        # ── Helper styles ──────────────────────────────────────────────────
        def para(txt, size=10, bold=False, color=colors.HexColor("#333"),
                 after=6, align=0):
            return Paragraph(txt, ParagraphStyle("p", parent=ss["Normal"],
                fontSize=size, fontName="Helvetica-Bold" if bold else "Helvetica",
                textColor=color, spaceAfter=after, alignment=align))
        ...
        story += [
            ...
            para("AI Academic Performance Tracker — Cohort Risk Report",
                 11, False, DGREY, after=4, align=1),
            para(f"Faculty Scope: {scope}", 10, False, DGREY, after=4, align=1),
            para(f"Prepared for: {role}  |  "
        ...
        summary_data = [
            ...
            ["Total Students Assessed", f"{n:,}",
             "Average Cohort GPA", f"{avg_gpa:.2f}"],
        ...
        if "faculty" in df.columns:
            ...
            fac_rows = [["Faculty", "Total", "High Risk", "Medium Risk",
                          "Low Risk", "High Risk %"]]
            for fac in FACULTIES:
                sub = df[df["faculty"]==fac]
                if len(sub) == 0:
                ...
                fac_rows.append([
                    fac, str(fn), str(int(fhr)), str(int(fmr)),
                    str(int(flr)), f"{fhr/fn*100:.0f}%"
                ])
            t = make_table(fac_rows, [3*cm, 2.2*cm, 2.5*cm, 2.5*cm, 2.5*cm, 2.3*cm])
            story.append(t)
            story.append(Spacer(1, 10))
        ...
        if "gender" in df.columns:
            ...
            g_rows = [["Gender", "Count", "Avg GPA", "High Risk",
            ...
            for g in genders:
                ...
                g_rows.append([g, str(fn), f"{gg:.2f}",
                               str(int(ghr)), str(int(gmr)), str(int(glr))])
            story.append(make_table(g_rows,
                                    [3.5*cm, 2.5*cm, 2.5*cm, 2.5*cm, 2.5*cm, 2.5*cm]))
            story.append(Spacer(1, 10))
        ...
        if "prob_high" in df.columns and "risk_class" in df.columns:
            top15 = (df[df["risk_class"]==2]
                     .sort_values("prob_high", ascending=False)
                     .head(15))
            if len(top15) > 0:
                show_cols = [c for c in ["student_id","name","faculty","semester_gpa",
                                          "prob_high"] if c in top15.columns]
                ...
                for i in range(1, len(t_data)):
                    ...
                    row_bgs.extend([("BACKGROUND",(0,i),(-1,i),c)])
        ...
        if insights:
            ...
            for i, (icon, color, headline, detail) in enumerate(insights, 1):
                story.append(para(f"<b>{i}. {headline}</b>",
                                  9, color=BLUE, after=3))
...
def main_dashboard(role: str):
    ...
    insights = generate_insights(df, faculty)
    if insights:
        with st.expander("🧠 AI Insights — What the data tells us", expanded=True):
            st.markdown(
                '<p style="color:#94a3b8;font-size:.82rem;margin-bottom:.8rem">'
                'These insights are automatically computed from the prediction '
                'results. Each finding is derived directly from your cohort data.</p>',
                unsafe_allow_html=True)
            i_cols = st.columns(2)
            for idx, (icon, color, headline, detail) in enumerate(insights):
                with i_cols[idx % 2]:
                    st.markdown(
                        '<div style="background:linear-gradient(160deg,#131a26 0%,'
                        '#0a0e17 100%);border:1px solid #2a3344;'
                        'border-left:4px solid ' + color + ';'
                        'border-radius:0 12px 12px 0;padding:.9rem 1.1rem;'
                        'margin:.35rem 0;height:100%">'
                        '<div style="font-weight:700;color:#f1f5f9;'
                        'font-size:.9rem;margin-bottom:.35rem">'
                        + icon + ' ' + headline + '</div>'
                        '<div style="color:#94a3b8;font-size:.81rem;'
                        'line-height:1.6">' + detail + '</div>'
                        '</div>',
                        unsafe_allow_html=True)
    ...
    insights = generate_insights(df, faculty)
    if insights:
        with st.expander("🧠 AI Insights — What the data tells us", expanded=True, key="insights_expander"):
            for icon, color, headline, detail in insights:
                st.markdown(f"""
                <div style="background:{_rgba(color,.08)};
                            border:1px solid {_rgba(color,.3)};
                            border-left:4px solid {color};
                            border-radius:0 12px 12px 0;
                            padding:.8rem 1rem;margin-bottom:.8rem;
                            font-size:.9rem;line-height:1.5">
                  <div style="color:{color};font-weight:700;margin-bottom:.3rem">
                    {icon} {headline}
                  </div>
    ...
    with st.expander("📊 View Detailed Analytics (Faculty Comparison, Gender, Trends, Graduation)",
        ...
        st.markdown(
            ...
            unsafe_allow_html=True)
        st.markdown(
            '<p style="color:#f1f5f9;font-size:.95rem;font-weight:700;'
            'margin-bottom:.4rem">📋 Export Cohort Report</p>',
            unsafe_allow_html=True)
        st.markdown(
            '<p style="color:#94a3b8;font-size:.82rem;margin-bottom:.8rem">'
            'Download a formatted PDF summary of this cohort — '
            'suitable for faculty meetings and institutional reporting.</p>',
            unsafe_allow_html=True)

        if st.button("📋 Generate Cohort PDF Report",
                     key="gen_cohort_pdf",
                     use_container_width=False):
            with st.spinner("Generating cohort report..."):
                cohort_pdf = generate_cohort_pdf(df, role, faculty)
            if cohort_pdf:
                st.session_state["cohort_pdf_cache"] = cohort_pdf
                st.success("✅ Report ready — click below to download.")
            else:
                st.info("Install `reportlab` to enable PDF export.")

        if st.session_state.get("cohort_pdf_cache"):
            fname = (f"cohort_report_{faculty or 'all'}_{datetime.date.today()}.pdf")
            st.download_button(
                "⬇️ Download Cohort Report PDF",
                data=st.session_state["cohort_pdf_cache"],
                file_name=fname,
                mime="application/pdf",
                key="dl_cohort_pdf",
                use_container_width=False,
